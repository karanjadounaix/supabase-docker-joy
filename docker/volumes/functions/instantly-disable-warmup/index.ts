// =============================================
// EDGE FUNCTION: instantly-disable-warmup
// =============================================
// Disables warmup for specified email accounts on Instantly.ai
// Called when inboxes are auto-changed to "Ready" status
// =============================================
...
interface DisableWarmupRequest {
    emails: string[];
}
...
Deno.serve(async (req: Request) => {
    ...
    try {
        ...
        console.log("INSTANTLY_API_KEY length:", INSTANTLY_API_KEY?.length || 0);
        ...
        const { emails }: DisableWarmupRequest = await req.json();
        ...
        console.log(`Disabling warmup for ${emails.length} emails...`);

        // Call Instantly API to disable warmup
        // API docs: https://developer.instantly.ai/api/v2/accounts/warmup/disable
        ...
        console.log(`Warmup disable job created:`, data);

        return new Response(
            JSON.stringify({
                success: true,
                disabled_count: emails.length,
                job_id: data.id,
                job_status: data.status,
                message: `Warmup disable initiated for ${emails.length} email(s)`,
            }),
            {
                headers: {
                    "Content-Type": "application/json",
